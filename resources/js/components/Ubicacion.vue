            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="h3 float-left">Adminstracion de Ubicaciones</h3>
                    ...
                    <div class="card-body ">
                            ...
                            <div class="col-4 d-flex flex-row-reverse">
                                <div class="input-group col-8 pr-0">
                                    ...
                                    <input type="text"  class="form-control"  
                                        @input="getUbicaciones()" v-model="search"
                        ...
                        <b-table class="col-12"
                            ...
                            <template v-slot:cell(accion)="row">
                                ...
                                <b-button size="sm" variant="danger" @click="deleteUbicacion(row.item.key_id)" >
            ...
            <div class="modal-dialog " role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" v-show="!editMode" id="createModalLabel">Crear Ubicacion</h5>
                        <h5 class="modal-title" v-show="editMode" id="createModalLabel">Actualizar Ubicacion</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form @submit.prevent="editMode === true ? updateUbicacion() : createCategory()">
...
<script>
    export default {
        ...
        methods:{
            getUbicaciones(url = base_url + '/api/ubicacion/'){
            ...
            createCategory(){          
                this.$Progress.start();  
                this.form.post('api/ubicacion')
                .then(()=>{            
                    Fire.$emit('AfterEvent');                           
                    $('#createModal').modal('hide');
                    toast.fire({
                        icon: 'success',
                        title: 'La ubicacion ha sido creada.'
                    }); 
            ...
            updateUbicacion(){
                this.$Progress.start();
                this.form.put('api/ubicacion/'+this.form.key_id)
                .then(()=>{
                    $('#createModal').modal('hide');
                    swal.fire(
                        'Actualizado!',
                        'La ubicacion ha sido actializada',
                        'success'
                    );
                    this.$Progress.finish();
                    Fire.$emit('AfterEvent'); 
            ...
            deleteUbicacion(key_id){
                swal.fire({
                    ...
                    if (result.value) { 
                        this.form.delete('api/ubicacion/'+key_id).then(()=>{ 
                            swal.fire(
                                'Eliminado!',
                                'La ubicacion ha sido eliminada.',
                                'success'
                            );
                            Fire.$emit('AfterEvent');    
                        }).catch(()=>{
